...
export default function Profile() {
  ...
  useEffect(() => {
    const fetchEmploye = async () => {
      try {
        const res = await axios.get("/employe/profile");
        setEmploye(res.data);
      } catch {
        setError("⚠️ Impossible de charger votre profil.");
      }
    };
    fetchEmploye();
  }, []);
  ...
  const handleProfileUpdate = async () => {
    try {
      setLoading(true);
      await axios.put("/employe/profile", employe);
  ...
  const handlePasswordUpdate = async () => {
    try {
      ...
      setLoading(true);
      await axios.put("/employe/password", { password });
